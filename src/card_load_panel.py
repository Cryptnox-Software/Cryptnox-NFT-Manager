...
class CardLoadPanel(ScrolledPanel):
    ...
    def __init__(self, parent, id):
        ...
        self.columns = ['Field 1', 'Field 2', 'PIN', 'PUK',
                        'Chain Name and ID', 'Contract address', 'Token ID', 'Metadata']
        self.SetBackgroundColour('black')
        self.SetForegroundColour('white')
        self.ABI_modes = ['Automatic', 'ERC-721 Openzeppelin',
                          'ERC-1155 Openzeppelin', 'Manual']
        ...
        self.manual_abi_sizer.Add(
            self.edit_abi, 0, wx.TOP | wx.RIGHT, border=5)
        ...
        text.SetFont(font)
        row_sizer.Add(text, 0, wx.ALL, border=5)
        self.endpoint_choice = wx.Choice(
            self, choices=[f'{key}: {value}' for key, value in self.endpoint_urls.items()])
        ...
        row_sizer = wx.BoxSizer(wx.HORIZONTAL)
        text = wx.StaticText(
            self, label='Please choose NFT data input method below:', size=(250, 16))
        text.SetFont(font)
        row_sizer.Add(text, 0, wx.ALL, border=5)

        self.clear_fields_btn = wx.Button(
            self, -1, label='Reset all fields', size=(100, 30))
        ...
        self.url_input_sizer.Add(
            self.get_fields, 0, wx.TOP | wx.RIGHT, border=5)
        ...
        for x in range(4, len(self.columns)):
            row_sizer = wx.BoxSizer(wx.HORIZONTAL)
            if x == 7:
                self.metedata_label = wx.StaticText(
                    self, label=self.columns[x], size=(100, 16))
                ...
                self.edit_metadata.Bind(
                    wx.EVT_BUTTON, self.edit_metadata_click)
                row_sizer.Add(field, 0, wx.ALL, border=5)
                row_sizer.Add(self.edit_metadata, 0,
        ...
        pub.subscribe(self.download_error, "download_error")
    ...
    def clear_fields(self, event):
        for i in range(1, len(self.columns)+1):
            ...
            self.endpoint_choice.SetItems(
                [f'{key}: {value}' for key, value in self.endpoint_urls.items()])
    ...
    def info_card(self, event='', message='', pin=''):
        ...
        try:
            ...
            wx.MessageBox(
                f"Card or reader not found, please ensure device is connected.\n\nError Information:\n\n{e}", "Error", wx.OK | wx.ICON_WARNING)
        ...
        try:
            ...
            result = utils._owner(
                slot0['endpoint'], slot0['contract_address'], abi, address, slot0['nft_id'])
            message += f'\n{result}'
            # message+='\n----------------------------------------------'
            ...
            if 'Connection' in str(e):
                if pin:
                    wx.MessageBox(
                        f"Could not get info from card due to connection loss. But Card has been initialized and ready for use.", "Error", wx.OK | wx.ICON_WARNING)
                else:
                    wx.MessageBox(
                        f"Could not get info from card, please ensure card is initialized.", "Error", wx.OK | wx.ICON_WARNING)
            else:
                wx.MessageBox(
                    f"Could not get info from card, please ensure card is initialized.", "Error", wx.OK | wx.ICON_WARNING)
            wx.CallAfter(pub.sendMessage, "start_check_card")
    ...
    def populate_fields(self, file_type, data):
        ...
        self.Parent.FindWindowById(5).SetValue(
            self.chains[endpoint]['id']+' ('+self.chains[endpoint]['name']+')')
    ...
    def file_picked(self, event: wx.EVT_FILEPICKER_CHANGED):
        ...
        if path_picked[-3:] == 'txt' and 'NFT' in path_picked:
            ...
            try:
                ...
                if 'polygon' in l[-2]:
                    self.endpoint_choice.SetStringSelection(
                        f'Polygon: {self.endpoint_urls["Polygon"]}')
                elif 'ethereum' in l[-2]:
                    self.endpoint_choice.SetStringSelection(
                        f'Ethereum: {self.endpoint_urls["Ethereum"]}')
                else:
                    wx.MessageBox(
                        "Unrecognized endpoint, please contact developer", "Info", wx.OK | wx.ICON_WARNING)
                    print(f'Unrecognized endpoint: {l[-2]}')
            except Exception as e:
                wx.MessageBox(
                    f"Please select a valid txt file with NFT fields.\n\n{e}", "Info", wx.OK | wx.ICON_WARNING)
                return
            ...
            wx.MessageBox("Please select a valid txt file with NFT fields.",
    ...
    def get_fields_from_url(self, event):
        ...
        self.endpoint_choice.SetStringSelection(
            f'{endpoint}: {self.endpoint_urls[endpoint]}')
        self.endpoint_choice.Disable()
        self.Parent.FindWindowById(8).SetValue('')

        ABI = self.fetch_ABI(
            self.abi_urls[endpoint], contract_address, self.api_keys[endpoint])
        self.ABI_chooser.SetStringSelection('Automatic')
        ...
        try:
            ...
            querystring = {
                "chain": endpoint.lower(), "refresh_metadata": "false"}
            headers = {
                'Content-Type': "application/json",
                'Authorization': self.metadata_apikeys['nftport']
            }
            nftport_response = requests.request(
                "GET", nftport_url, headers=headers, params=querystring)
        ...
        try:
            ...
            wx.MessageBox(
                f"Network error in fetching Metadata, please input manually or try again.\n\nError Information:\n\n{e}", "Error", wx.OK | wx.ICON_WARNING)
            self.Parent.FindWindowById(8).SetEditable(True)

    def fetch_ABI(self, url, contract_address, api_key):
        try:
            response = requests.get(
                url+f'&address={contract_address}'+f'&apikey={api_key}')
            ...
            if 'Invalid' in str(e) or 'verified' in str(e):
                wx.MessageBox(
                    f"ABI could not be fetched, please input manually or try again.\n\nInfo:\n\n{e}", "Info", wx.OK | wx.ICON_WARNING)
            else:
                wx.MessageBox(
                    f"Network error in fetching ABI, please input manually or try again.\n\nError Information:\n\n{e}", "Error", wx.OK | wx.ICON_WARNING)
            self.manual_ABI.SetEditable(True)
            self.manual_ABI.Enable()
            resp = ''
        return resp
    ...
    def execute_load(self, event):
        ...
        d['chain_id'] = int(
            self.chains[self.endpoint_choice.GetStringSelection().split(':')[0]]['id'])
        ...
        try:
            ...
            if not ipfs and 'mp4' not in meta['image_url']:
            ...
            if not ipfs and 'mp4' not in meta['image']:
                image_url = f"{GATEWAY_URL}/{meta_url[-2]}/{meta_url[-1]}"
                meta['image'] = image_url
        slots[3] = json.dumps(meta)
        try:
            card.init(data['Name'], data['Mail'], data['PIN'],
                      data['PUK'], nfc_sign=self.NFC_choice.GetValue())
        except Exception as e:
            print(
                f'Exception in init: {e}\n\nPlease reset the card and try again.')
            if 'Connection' in str(e):
                wx.MessageBox(
                    f"Connection Issue.\n\nError Information:\n\n{e}", "Error", wx.OK | wx.ICON_WARNING)
            else:
                wx.MessageBox(
                    f"Card cannot be initialized.\n\nError Information:\n\nCard is already initialized, please reset the card.", "Error", wx.OK | wx.ICON_WARNING)
                print(e)
            wx.CallAfter(pub.sendMessage, "start_check_card")
            return
        try:
        ...
        try:
            for index, value in enumerate(slots):
                if value:
                    card.user_data[index] = gzip.compress(
                        value.encode("UTF-8"))
        except Exception as e:
            print(f'Error writing data to card: {e}')
            wx.MessageBox(
                f"Error writing data to card.\n\n {e}", "Error", wx.OK | wx.ICON_WARNING)
            wx.CallAfter(pub.sendMessage, "start_check_card")
            return
        print('Data written')
    ...
    def get_card(self):
        ...
        while index < 5:
            ...
            index+=1
        raise Exception('NFT Card not found')
    ...
    def validate_fields(self):
        ...
        if 5 > len(self.Parent.FindWindowById(3).GetValue()) < 9:
            wx.MessageBox("PIN must be more than 5 and less than 9 numeric values",
                          "Error", wx.OK | wx.ICON_INFORMATION)
        if len(self.Parent.FindWindowById(4).GetValue()) <= 11:
            wx.MessageBox("PUK must be 12 alphanumeric value",
                          "Error", wx.OK | wx.ICON_INFORMATION)
    ...
    def reset_card(self, event):
        try:
            card = self.get_card()
            puk_result = utils.ask(
                message='Please input your PUK to reset:\n\nFor EASY PUK "000000000000", press enter.')
            if type(puk_result) != str:
                return
            puk = '000000000000' if puk_result == '' else puk_result
            confirm = utils.confirm(
                message='This will reset the card, are you sure ?')
            if confirm == 5100:
                # wx.CallAfter(pub.sendMessage,"pause_check_card")
                pub.sendMessage("pause_check_card")
                card.reset(puk)
                wx.MessageBox(f"Card has been reset.", "Info",
    ...
    def metadata_changed(self, event):
        ...
        if v:
            try:
                ...
                image_url = metadata_json['image_url'] if 'image_url' in metadata_json.keys(
                ) else metadata_json['image']
                ...
                if 'content-length' in str(e):
                    wx.MessageBox(
                        f"Error downloading NFT preview.\n\nPossible IPFS gateway failure, please consider changing.\n\nError info: \n{e}", "Error", wx.OK | wx.ICON_WARNING)
                else:
                    wx.MessageBox(
                        f"Error downloading NFT preview.\n\nError information:\n{e}", "Error", wx.OK | wx.ICON_WARNING)
        else:
    ...
    def download_error(self, data):
        if 'content-length' in data:
            wx.MessageBox(
                f"Error downloading NFT preview.\n\nPossible IPFS gateway failure, please consider changing.\n\nError information:\n{data}", "Error", wx.OK | wx.ICON_WARNING)
        else:
            wx.MessageBox(
                f"Error downloading NFT preview.\n\nError information:\n{data}", "Error", wx.OK | wx.ICON_WARNING)
    ...
    def downloaded(self, data):
        ...
        try:
            ...
            if not file_type.startswith('image') and file_type != 'video/mp4':
                wx.MessageBox(f"File format not recognized, preview unavailable",
                              "Error", wx.OK | wx.ICON_INFORMATION)
                return
    ...
    def show_nft(self, nft_type, data):
        ...
        if nft_type == 'gif' or nft_type == 'mp4':
            ft = tempfile.NamedTemporaryFile(
                delete=False, suffix=f".{nft_type}")
            ft.write(data)
            ft.flush()
            ft.close()
            if nft_type == 'mp4':
                self.anim = media.MediaCtrl(self, -1, style=wx.SIMPLE_BORDER, pos=(
                    0, 0), size=(250, 250), szBackend=wx.media.MEDIABACKEND_WMP10)
            else:
                self.anim = media.MediaCtrl(
                    self, -1, style=wx.SIMPLE_BORDER, pos=(0, 0), size=(250, 250))
    ...
    def scale_bitmap(self, bitmap, width, height):
        ...
        rescale_height = ((bitmap.GetWidth()-250) /
...
class MessageBox(wx.Dialog):
    def __init__(self, parent, title, message):
        ...
        for each in message.split('\n'):
            text = wx.TextCtrl(window, style=wx.TE_READONLY |
                               wx.BORDER_NONE, pos=(30, height), size=(600, 23))
            text.SetValue(each)
    ...
    def __init__(self, parent, title, field, read_only=False):
        ...
        if read_only:
            self.text = wx.TextCtrl(
                window, style=wx.TE_MULTILINE | wx.TE_READONLY, size=(400, 400))
        else:
            self.text = wx.TextCtrl(
                window, style=wx.TE_MULTILINE, size=(400, 400))
        ...
        if not read_only:
            save_btn = wx.Button(
                self, -1, label="Save changes", size=(100, 30))
